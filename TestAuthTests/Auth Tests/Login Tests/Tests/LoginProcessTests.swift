...
class LoginProcessTests: XCTestCase {
    ...
    func test_Login_withValidData_SuccessLogin() {
        // Given
    ...
    func test_Login_withSetWrongPassword_FailLogin() {
        // Given
        let user = UserStubs.createUser()
        user.password = "123"
        let response = Result.success(user)
    ...
    func test_Login_withSetUserNotFound_FailLogin() {
        // Given
        let response = Result<UserModel>.failure(.userNotFound)
        // When
        let result = stub.login(response, password: "123422256")
        // Then
        switch result {
        case .success(_):
            XCTAssertThrowsError("There are some error in login func must be fail login")
